...
class RadioPlayer extends Simulation {
  ...
  val liveStreamData = csv("radioplayer/live-stream.txt").circular
  ...
  val scn = scenario("Radio Player")
    ...
    .randomSwitch(
      ...
      17d -> exec(http("On Demand Stream").get("/b05r6hw8") // episodePID
        .headers(flagpoleHeaders)
        .check(status.is(200))),
      ...
      7d -> exec(http("More Episodes Feed").get("/b05r6hw8/episodes/1") // episodePID & page
        .headers(flagpoleHeaders)  
        .check(status.is(200))),

      7d -> exec(http("More Like This Feed").get("/b05r6hw8/morelikethis/1") // episodePID & page
        .headers(flagpoleHeaders)  
        .check(status.is(200))),
      ...
      3d -> exec(http("Favourites Feed").get("/b05r6hw8/favourites") 
        .headers(flagpoleHeaders)  
        .check(status.is(200))),
